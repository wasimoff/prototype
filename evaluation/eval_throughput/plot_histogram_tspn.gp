...
FILE = "results/histogram_tspn.csv"
set output "plots/histogram_tspn".ext;
set title "nbg1-fsn1-fsn1, 32x02c, 64 users"
set yrange [5:5000]
set y2range [5:5000]
set key at screen 0.20,0.88
set logscale y
set logscale y2
eWasi = 1; eServ = 2;
plot \
  FILE every 2::eWasi  using 8  axes x1y2 title "" with lines smooth mcs lc 4 lw 3 , \
  FILE every 2::eServ  using 8  axes x1y2 title "" with lines smooth mcs lc 6 lw 3 , \
  FILE every 2::eWasi  using 4:xtic(2)   title "Wasimoff"               lc 4      , \
  FILE every 2::eServ  using 4           title "Serverledge"            lc 6      

# # ---> replot for tsp-8
# set output "plots/histogram_tspn_tsp-8".ext;
